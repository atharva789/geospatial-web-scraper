...
// now the project will be run via docker

// const envFilePath = "/Users/thorbthorb/Downloads/geospatial-web-scraper/internal.env"
...
func DataQuery(prompt, query, data string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("Error converting LLMMessage to JSON. %v", err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("error while copying groq-api response body: %s", err)
	}
...
func CompareEntities(prompt, query, dataOne, dataTwo string) (string, error) {
	data := fmt.Sprintf("1: %v\n 2:%v", dataOne, dataTwo)
	out, err := DataQuery(prompt, query, data)
	if err != nil {
		return "", fmt.Errorf("	(CompareEntities Error): %v", err)
	}
